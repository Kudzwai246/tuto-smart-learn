-- Fix trigger creation as they don't have IF NOT EXISTS support
...
-- Triggers with guards since they don't support IF NOT EXISTS
DO $$
BEGIN
  BEGIN
  ...
  END;
  BEGIN
    CREATE TRIGGER set_teacher_documents_updated_at BEFORE UPDATE ON public.teacher_documents
    FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
  EXCEPTION
    WHEN duplicate_object THEN null;
  END;
